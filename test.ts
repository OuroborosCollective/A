...
test("canonicalizeUrl removes tracking data and sorts retained parameters", () => {
  assert.equal(canonicalizeUrl("HTTPS://Example.COM:443/path/?utm_source=x&b=2&a=1#fragment"), "https://example.com/path?a=1&b=2")
})
...
test("assessEvidence never promotes an archived mirror to primary evidence", () => {
  assert.equal(assessEvidence({ sourceType: "Webarchiv", hasOriginal: false, hasArchive: true, recordHashVerified: true, signatureVerified: false, independentConfirmations: 0, contradictedByPrimaryEvidence: false }).tier, "Unabhängig archiviert")
})

test("primary contradiction wins over secondary confirmations", () => {
  assert.equal(assessEvidence({ sourceType: "Medienbericht", hasOriginal: false, hasArchive: true, recordHashVerified: true, signatureVerified: false, independentConfirmations: 5, contradictedByPrimaryEvidence: true }).tier, "Widerlegt")
})

test("deriveResearchPaths returns six explicit paths", () => {
  const paths = deriveResearchPaths({ title: "Bitcoin whitepaper", sourceType: "PDF", url: "https://bitcoin.org/bitcoin.pdf" })
  assert.equal(paths.length, 6)
  assert.match(paths[0] ?? "", /früheste Version/)
})

test("calculateHype marks attention without primary evidence", () => {
  const result = calculateHype({ mentionCount: 100, independentPublishers: 20, searchTrend: 100, priceVolatility: 20, primaryEvidenceCount: 0 })
...
test("Wikipedia parser binds a specific revision and reference graph", () => {
  const page = parseWikipediaQuery({ query: { pages: [{ pageid: 123, title: "Satoshi Nakamoto", revisions: [{ revid: 456, timestamp: "2026-08-01T12:00:00Z" }], extlinks: [{ url: "https://bitcoin.org/bitcoin.pdf" }, { url: "https://example.org/source" }], langlinks: [{ lang: "de", title: "Satoshi Nakamoto", url: "https://de.wikipedia.org/wiki/Satoshi_Nakamoto" }] }] } }, { language: "en", title: "Satoshi Nakamoto" })
...
test("Common Crawl parser preserves WARC provenance and early Bitcoin-era timestamp", () => {
  const [capture] = parseCommonCrawlIndex(JSON.stringify({ url: "http://www.bitcoin.org/", timestamp: "20090702152548", digest: "sha1:ABC", status: "200", mime: "text/html", filename: "crawl-data/CC-MAIN-2009-2010/segments/example.warc.gz", offset: "100", length: "200", languages: "eng" }), "CC-MAIN-2009-2010")
  assert.ok(capture)
...
test("Bitcointalk parser attributes u=3 post without quoted text", () => {
  const html = `<div id="bodyarea"><div class="windowbg"><div class="poster_info"><b><a href="https://bitcointalk.org/index.php?action=profile;u=3">satoshi</a></b></div><div class="td_headerandpost"><table><tr><td class="smalltext">December 13, 2010, 04:45:41 PM</td></tr></table><a href="https://bitcointalk.org/index.php?topic=2228.msg29479#msg29479">Re: Wikileaks</a><div class="post"><div class="quoteheader">Quote from somebody</div><div class="quote">This must not be attributed.</div>I make this appeal to Wikileaks not to try to use Bitcoin. Bitcoin is a small beta community in its infancy.</div><div class="signature">signature</div></div></div></div>`
...
test("Bitcointalk parser accepts legacy showPosts table rows", () => {
  const html = `<table><tr class="catbg"><td colspan="2" align="left" class="smalltext"><div style="float: right;">Posted on: July 25, 2010, 10:06:57 PM</div>Posted by: satoshi</td></tr><tr class="windowbg2"><td class="smalltext" id="msg12345"><div align="right" onclick="return insertQuoteFast(12345);">Insert Quote</div><div class="post">For future reference, here is my public key. Check the fingerprint and signatures independently.</div></td></tr></table><a href="https://bitcointalk.org/index.php?action=profile;u=3;sa=showPosts;start=10">next</a>`
...
test("Bitcointalk parser binds deeply nested showPosts cells by nearest msg id", () => {
  const html = `<div id="bodyarea"><table><tr><td><table><tr><td><table><tr><td>Topic: <a href="index.php?topic=48.0">Re: Transaction volume</a></td></tr><tr><td>Posted on: February 14, 2010, 11:15:12 PM</td></tr><tr><td id="msg329"><span onclick="return insertQuoteFast(329);">quote</span><div class="post">I'm sure that in 20 years there will either be very large transaction volume or no volume.</div></td></tr><tr><td>separator unrelated to the post</td></tr></table></td></tr></table></td></tr></table></div>`
  const posts = parseBitcointalkPosts(html, { author: "satoshi", authorId: "3" })
...
function forumSource(summary: string): ResearchSource {
  return { canonicalId: "bitcointalk-message:0123456789abcdef", title: "Forum statement", lane: "Satoshi Forum", sourceType: "Forum-Post", evidenceTier: "Primär belegt", originalUrl: "https://bitcointalk.org/index.php?topic=1.0#msg1", publishedAt: "2010-12-13T16:45:41.000Z", retrievedAt: "2026-08-07T14:00:00.000Z", upstreamId: "msg:1;topic:1;user:3", recordSha256: "abc", contentHashVerified: false, adapter: "Bitcointalk HTML", status: "In Prüfung", subjects: ["Satoshi", "Bitcoin", "Kryptografie", "Identität"], summary, primarySource: true, independentConfirmations: 0 }
}
...
test("SourceForge project statements become open claims and source-specific paths", () => {
  const source: ResearchSource = { canonicalId: "sourceforge-bitcoin:abc", title: "Bitcoin v0.1 released - P2P e-cash", lane: "SourceForge", sourceType: "SourceForge-Projektmeldung", evidenceTier: "Primär belegt", originalUrl: "https://sourceforge.net/p/bitcoin/news/2009/01/bitcoin-v01-released---p2p-e-cash/", publishedAt: "2009-01-13T00:00:00.000Z", retrievedAt: "2026-08-07T15:00:00.000Z", upstreamId: "release-v0.1", recordSha256: "def", contentHashVerified: false, adapter: "SourceForge public HTML", status: "In Prüfung", subjects: ["Satoshi", "Bitcoin", "Historie", "Technik"], summary: "Historischer SourceForge-Projektrecord. Primärevidenz gilt für die Veröffentlichung/Projektaktivität auf SourceForge, nicht automatisch für jede darin enthaltene Sachbehauptung. Inhalt: Announcing the release of Bitcoin, a new electronic cash system that uses a peer-to-peer network to prevent double-spending. The software is still alpha and experimental, with no guarantee that the system state will never need to be restarted.", primarySource: true, independentConfirmations: 0 }
  const claims = extractClaimCandidates(source)
...
test("Wikipedia remains a reference graph and generates archive follow-up rather than primary proof", () => {
  const source: ResearchSource = { canonicalId: "wikipedia-revision:abc", title: "Satoshi Nakamoto (en.wikipedia)", lane: "Wikipedia Reference Graph", sourceType: "Wikipedia-Revision-und-Referenzgraph", evidenceTier: "Behauptet", originalUrl: "https://en.wikipedia.org/wiki/Satoshi_Nakamoto", archiveUrl: "https://en.wikipedia.org/wiki/Satoshi_Nakamoto?oldid=123", publishedAt: "2026-08-01T00:00:00Z", retrievedAt: "2026-08-07T15:00:00Z", upstreamId: "page:1;revision:123;lang:en", recordSha256: "ghi", contentHashVerified: false, adapter: "MediaWiki Action API", status: "In Prüfung", subjects: ["Satoshi", "Bitcoin", "Historie", "Referenzgraph"], summary: "Wikipedia wird nur als Sekundärquelle/Referenzgraph verwendet. Externe Referenzen: https://bitcoin.org/bitcoin.pdf", primarySource: false, independentConfirmations: 0 }
...
test("historical discovery keeps unstable direct SourceForge origin out of the free cron", () => {
  assert.deepEqual(HISTORICAL_DISCOVERY_PROVIDERS, ["wayback", "wikipedia", "commoncrawl"])
})

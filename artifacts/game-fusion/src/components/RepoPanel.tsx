...
function StatCard({ title, count, accent, description }: { title: string, count: number, accent: string, description?: string }) {
  const isCyan = accent === 'cyan';
  const containerClasses = `p-4 rounded-lg border text-center flex flex-col items-center justify-center bg-black/60 shadow-inner h-full w-full ${isCyan ? 'border-cyan-900/50' : 'border-fuchsia-900/50'}`;
  ...
  if (!description) return <div className={containerClasses}>{content}</div>;
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          type="button"
          className={`${containerClasses} cursor-help focus-visible:ring-2 focus-visible:ring-primary outline-none`}

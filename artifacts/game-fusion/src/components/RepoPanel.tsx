...
function StatCard({ title, count, accent, description }: { title: string, count: number, accent: string, description?: string }) {
  ...
  const content = (
    <button
      type="button"
      aria-label={`${title}: ${count}${description ? '. ' + description : ''}`}
      className={`p-4 rounded-lg border text-center flex flex-col items-center justify-center bg-black/60 shadow-inner h-full w-full transition-all outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary ${isCyan ? 'border-cyan-900/50' : 'border-fuchsia-900/50'} ${description ? 'cursor-help' : 'cursor-default'}`}
    >

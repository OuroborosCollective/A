...
import { fusionLimiter } from "../../middlewares/rateLimiter";
...
router.post("/fusion/fetch-repo", async (req, res): Promise<void> => {
  ...
  try {
    const { defaultBranch, description, tree } = await fetchRepoTree(
      owner,
      repo,
    );
    ...
    req.log.info(
      { owner, repo, totalFiles, toFetch: toFetch.length },
      "Fetching repo files",
    );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      const result = await analyzeGameRepo(
        repoData,
        role as "graphics" | "logic",
      );
      res.json(result);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Unknown error";
...
router.post("/fusion/fuse", fusionLimiter, async (req, res): Promise<void> => {
  ...
  try {
    req.log.info(
      { repoA: gameA.repoData.repo, repoB: gameB.repoData.repo },
      "Fusing games",
    );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const zipName = `fused-${gameAName}-x-${gameBName}.zip`.replace(
      /[^a-zA-Z0-9\-_.]/g,
      "_",
    );

    req.log.info(
      { files: fusionResult.files.length, zipName },
